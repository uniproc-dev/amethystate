...
impl AmeBackendSync for SyncBridge {
    ...
    fn set_with_source<T: Serialize>(
        ...
        Ok(self.store.set_with_source(path, value, source)?)
    }
    ...
    fn set_owned_with_source<T: Serialize>(
        ...
        Ok(self.store.set_owned_with_source(path, value, source)?)
    }
    ...
    fn decode<T>(&self, raw: &[u8]) -> StorageResult<T>
    where
